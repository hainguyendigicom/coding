...
public class SearchProductController
{
    ...
    public Object search(@RequestParam(name = "customerGroupId", required = false) final String customerGroupId,
        ...
        if(StringUtils.isNotEmpty(customerGroupId) && !customerGroupId.equals("0")){
            uriVariables.put("customerGroupId", customerGroupId);
        } else {
            uriVariables.put("customerGroupId", null);
        }
        uriVariables.put("query", query);
        uriVariables.put("currentPage", currentPage);
        uriVariables.put("pageSize", pageSize);
        uriVariables.put("sort", sort);
        return restTemplate.getForEntity(searchServiceUrl + "/products/search?customerGroupId={customerGroupId}&query={query}&currentPage={currentPage}&pageSize={pageSize}&sort={sort}",
